# Platform: Dell MX7000 blades + storage rack — the production hardware ($150,000 + $50,000)

*Hosting-platform page.* Two ways to run HCloud on this hardware, both using the same
middleware: **bare-metal K3s** (one `platform/linux/10-host-prep.sh` per sled, then
`hcloud/` with `HCLOUD_HA=true` on the first server — fastest to production) or **K3s on
OpenStack VMs** (`docs/platforms/openstack.md`, adds collaborator self-service). The
purchasing summary below is unchanged.
...
| Manifests | NERC OpenShift export → `apps/nerc-migration/20-convert-manifests.py` (Routes → Ingress) |
| Ingress / TLS | ingress-nginx + cert-manager (Let's Encrypt) + in-cluster Cloudflare Tunnel (`hcloud/12`, `hcloud/13`) |
| Storage classes | `local-path` (`hcloud/11-storage.sh`) pinned to the NVMe tier for databases; Longhorn across sleds where replicated PVs are wanted (platform add-on); MinIO on the SAS tier |
| GPU | NVIDIA device plugin / GPU Operator on the GPU node |
| Monitoring | kube-prometheus-stack (`hcloud/15-monitoring.sh`) |
...
proven on the Linux PoC box. CRC/OpenShift remains only in the Mac proof-of-concept (`platform/mac/crc/`).
