...
The Mac Studio (Apple Silicon Ultra, 128 GiB RAM, 20 TiB on `/Volumes/HSZ`) is a
**proof-of-concept** hosting platform. Apple Silicon cannot run Linux containers
natively, so HCloud on a Mac is always HCloud inside a Linux VM. Two flavours exist:

| Flavour | Directory | Status |
|---|---|---|
| **K3s middleware in a Lima VM** — the standard HCloud path | `platform/mac/10-lima-vm.sh`, then `hcloud/` inside the VM | ⬜ reference, not yet exercised |
| **OpenShift Local (CRC)** — OpenShift-API parity with NERC, no middleware | `platform/mac/crc/` | ✅ cluster up 2026-07-28; superseded by K3s |

The Lima path: `bash platform/mac/10-lima-vm.sh` creates an Ubuntu VM (16 vCPU / 96 GiB /
4 TiB sparse disk, repo and `/Volumes/HSZ` mounted), then inside it the README quick start
applies unchanged. It cannot offer GPUs (Metal is not exposed to Linux guests) and is a
single node; otherwise it is a normal HCloud.

The rest of this page is the validated **CRC runbook**, kept because it works and because
it documents NERC-service parity in OpenShift terms.

---

## CRC flavour — ordered path from a fresh macOS to the migrated `favor-4ee4be` workloads
...
Each script is idempotent. All install/state goes onto `/Volumes/HSZ`. Scripts live in `platform/mac/crc/`.

| # | Script | What it does | Approx time |
|---|---|---|---|
| 10 | `10-prereqs.sh` | Homebrew + `crc` + `oc` + `helm` + `cloudflared` + `httpd`/`htpasswd` + `jq` | 5–10 min |
| 11 | `11-install-crc.sh` | Symlinks `~/.crc` → `/Volumes/HSZ/.crc`; profiles CRC (16 vCPU / 96 GiB / 4 TiB); `crc setup` + `crc start` | 30–60 min (bundle download + boot) |
| 12 | `HCLOUD_TUNNEL_NAME=… HCLOUD_DOMAIN=… 12-cloudflared-tunnel.sh` | Cloudflare Tunnel as launchd, wildcard `*.<domain>` → CRC router | 2 min |
| 13 | `HCLOUD_CF_API_TOKEN=… HCLOUD_ACME_EMAIL=… HCLOUD_DOMAIN=… 13-cert-manager.sh` | cert-manager + Let's Encrypt wildcard cert | 5 min (DNS propagation) |
| 14 | `14-storage-parity.sh` | Adds a StorageClass alias `ocs-external-storagecluster-ceph-rbd`; MinIO operator | 2 min |
| 15 | `HCLOUD_USERS='alice bob' 15-users-quotas.sh` | HTPasswd IdP + per-user project + ResourceQuota + LimitRange | 2 min |
| 16 | `16-nerc-services.sh` | OpenShift Pipelines + Serverless + OpenDataHub (RHOAI upstream) | 10 min (operators install async) |
| 17 | `17-migrate-to-crc.py --ns favor-4ee4be` | Convert NERC raw exports → apply-ready CRC manifests | 5 sec |
...
| 18 | `NERC_KUBECONFIG=… LOCAL_KUBECONFIG=… 18-rsync-nerc-data.sh favor-4ee4be` | Copy every PVC's data via helper pods, NERC → local | **hours** (≈8 TB) |
| — | `HCLOUD_MANIFEST_VARIANT=crc apps/nerc-migration/50-scale-up.sh favor-4ee4be` | Restore original replica counts from `_replicas.json` | 30 sec |
...
- Then apply the un-rewritten route: run `17-migrate-to-crc.py` with an empty
  `HCLOUD_DOMAIN` to keep hosts, or hand-edit `30-routes.yaml` before apply.

## Honest limits vs NERC (things this Mac cannot do, either flavour)
...
3. **CILogon federated login is not wired.** Users log in with local htpasswd
   (script 15). Wire Keycloak+CILogon after public serving is up (needs a
...
1. Run 10 → 16 on the Mac. Verify `oc get nodes` shows the local cluster Ready.
2. Run 17 to convert manifests. Run `oc apply -f .../crc/` — pods come up at
   replicas: 0, PVCs bind.
3. Kick off 18 in a nohup / tmux — it will run for many hours copying the
   PVCs. Use `du -sh /Volumes/HSZ/.crc/machines/crc/` to watch disk grow.
4. Optional: run 18 again for a fast incremental catchup right before cutover.
5. On NERC: scale everything to 0 (`oc scale --replicas=0 --all`).
6. On the Mac: run the scale-up (see table) to scale up. Verify each Route responds on
   `<name>.<HCLOUD_DOMAIN>`.
