#!/usr/bin/env bash
# Application layer · 41 — sharded parallel tar copy of a BIG live volume (immutable-file stores: ClickHouse, RocksDB).
# One tar stream per 2nd-level subdir, PAR in parallel, each shard retried; resumable at shard grain.
# Usage: PAR=6 41-migrate-bigvol.sh <name> <nerc_pod> <nerc_path> <local_pvc> [nerc_container]
set -uo pipefail
...
NSA=("$OC" --kubeconfig "$MIG/nerc-sa.kubeconfig" -n "$NS" exec); [ -n "$NCON" ] && NSA+=(-c "$NCON")
K=(kubectl -n "$NS"); MOVER="mover-$NAME"
...
export OC MIG NPOD NPATH NCON MOVER NS
copy_one(){
  s="$1"
  OCC=("$OC" --kubeconfig "$MIG/nerc-sa.kubeconfig" -n "$NS" exec); [ -n "$NCON" ] && OCC+=(-c "$NCON")
  for a in 1 2 3; do
    set -o pipefail
    if "${OCC[@]}" "$NPOD" -- tar cf - -C "$NPATH" "$s" 2>/dev/null \
        | kubectl -n "$NS" exec -i "$MOVER" -- sh -c "mkdir -p /dest/\"\$(dirname '$s')\"; tar xf - -C /dest"; then
      return 0; fi
    sleep 20
  done
