...
**Where this sits in HCloud:** this is a *hosting platform* (`platform/openstack/`). It
ends where every platform ends — Linux VMs with a data disk and `hcloud/hcloud.env` — and
the K3s middleware in `hcloud/` runs unchanged inside them (§13–14 below map onto
`hcloud/10-install-k8s.sh` with `HCLOUD_K8S_EXTRA_ARGS`, and the platform add-on
`platform/openstack/20-cloud-integration.sh`). Bare-metal K3s on the same sleds needs only
`platform/linux/` and is the rollback (§17).
...
`platform/openstack/main.tf` (the repo's copy also adds the security group and per-server data volumes):
...
**Keep the NERC StorageClass name working.** `hcloud/11-storage.sh` creates the alias
`ocs-external-storagecluster-ceph-rbd` → `local-path`; `platform/openstack/20-cloud-integration.sh`
re-points it at `cinder-ceph` so unmodified manifests apply.
...
Cloudflare Tunnel and cert-manager DNS-01 are the middleware's `12-ingress-tls.sh` and
`13-public-tunnel.sh` — the tunnel runs as a pod, so it neither knows nor cares that the
node is a VM.
...
1. `apps/nerc-migration/10-export-nerc.sh` — export `favor-4ee4be` (already done).
2. `apps/nerc-migration/20-convert-manifests.py` — Routes → Ingress, `local-path`/`cinder-ceph`.
3. `apps/nerc-migration/30-apply.sh` — structure with `replicas: 0`.
4. Copy data — `40-migrate-volume.sh` / `41-migrate-bigvol.sh`, or `60-resume.sh` from the
   staging copy on `cluster-data/`.
5. Delta-sync, `apps/nerc-migration/50-scale-up.sh`, rotate NERC tokens.
...
1. `hcloud/10-install-k8s.sh` (K3s or RKE2) is unchanged and targets bare metal via `platform/linux/`.
...
migrated NERC stack on bare-metal K3s per `docs/platforms/dell-mx7000.md`.
...
Repo layout for everything this document creates: `platform/openstack/` (Terraform, cloud-init,
the Cinder/CCM add-on) and, when the Kolla deploy starts, `platform/openstack/kolla/`
(`globals.yml`, inventory, `config/` overrides — never `passwords.yml`).
