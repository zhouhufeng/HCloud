#!/usr/bin/env bash
# Mac Studio · CRC/OpenShift flavour · 11 — configure and start CRC (OpenShift Local) on
# Apple Silicon with the mac-studio profile.
#
# Profile — mac-studio: 16 vCPU / 96 GiB (98304 MiB) / 500 GiB disk.
# Proof-of-concept OpenShift cluster; enables cluster monitoring
# for NERC-style Prometheus/Grafana on day 1.
#
# Requires:
#   - platform/mac/crc/10-prereqs.sh finished (crc + oc + helm etc. installed).
...
command -v crc >/dev/null   || { echo "ERROR: crc missing — run platform/mac/crc/10-prereqs.sh"; exit 1; }
...
Next    : platform/mac/crc/12-cloudflared-tunnel.sh   (public serving)
          platform/mac/crc/13-cert-manager.sh         (real TLS certs)
          platform/mac/crc/14-storage-parity.sh       (NERC storage-class alias + MinIO)
          platform/mac/crc/15-users-quotas.sh         (NERC-style users + quotas)
          platform/mac/crc/16-nerc-services.sh        (OpenDataHub / Serverless / Pipelines)
          platform/mac/crc/17-migrate-to-crc.py       (apply your NERC exports to this cluster)
EOF
