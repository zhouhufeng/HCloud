#!/usr/bin/env bash
# Mac Studio · CRC/OpenShift flavour · 10 — install the tooling CRC + public serving need on macOS.
#
# Target machine: Apple Silicon Mac (M-series Ultra), 128 GiB RAM. This chain is the
# OpenShift-flavoured HCloud proof-of-concept; the K3s middleware (hcloud/) is the
# production path. Idempotent; safe to re-run.
#
# What it does:
...
Prereqs OK. One manual step before platform/mac/crc/11-install-crc.sh:
...
EOF
  ...
  echo
  echo "Prereqs OK. Pull secret found. Next: platform/mac/crc/11-install-crc.sh"
fi
