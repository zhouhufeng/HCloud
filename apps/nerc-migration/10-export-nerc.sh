#!/usr/bin/env bash
# Application layer · 10 — export everything from a NERC OpenShift namespace before the platform terminates.
# Usage: oc login <NERC API URL> first, then: ./10-export-nerc.sh <namespace> [more namespaces...]
# Output lands in docs/Secretes/migration/<namespace>/raw/ (git-ignored) for 20-convert-manifests.py.
set -euo pipefail
